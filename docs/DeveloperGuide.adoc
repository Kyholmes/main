...
`DeleteAppointmenetCommandParser` will verify if the input given is in correct format. If the input is valid, `DeleteAppointmenetCommand` will then call for the utility method `deletePatientAppointment` under `ModelManager`, to delete the appointment from the appointment list of Bernice. The updated appointment list is shown in Figure 32 below.
...
Suppose that the user has just launched the application. The current patient queue is shown in Figure 33 below.
...
details of the patient's current record as shown in Figure 34 below.
...
which will be edited based on the user's input into the medical record window. This new patient will be stored as the edited patient as shown in Figure 35 below.
...
the new list will be as shown in Figure 36 below.
...
.  Set the destination to `Save as PDF`, then click `Save` to save a copy of the file in PDF format. For best results, use the settings indicated in Figure 37 below.
